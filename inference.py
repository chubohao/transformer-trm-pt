...
def tokenizer_vacab(sentences):
    ...
    return enc_vocab, dec_vocab, dec_vocab['sos'], enc_vocab["pad"], dec_vocab['eos']

def tokenizer_source(source_sentence, enc_vocab):
   token_ids = [enc_vocab[word] for word in source_sentence.split()]
   token_ids = torch.tensor(token_ids)
   token_ids = torch.unsqueeze(token_ids, 0)
   return token_ids

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
model = torch.load("/homes/bohaochu/project/transformer-trm-pt/model/model.pth")
...
enc_vocab, dec_vocab, sos_id, enc_pad, dec_pad = tokenizer_vacab(sentences)
source = "ich mochte ein bier"
token_ids = tokenizer_source(source, enc_vocab).to(device)
x_input_cpu = token_ids.cpu().numpy()
words = [list(enc_vocab.keys())[list(enc_vocab.values()).index(idx)] for idx in x_input_cpu[0]]
# 拼接单词以形成句子
sentence = ' '.join(words)
print(sentence)
print(token_ids)
y_input = torch.tensor([[sos_id]], dtype=torch.long, device=device)
for _ in range(10):
  pred, encoder_attns, decoder_attns, decoder_encoder_attns = model(token_ids, y_input)
  next_item = pred.topk(1)[1].view(-1)[-1].item()
  next_item = torch.tensor([[next_item]], device=device)
  y_input = torch.cat((y_input, next_item), dim=1)
  if next_item.view(-1).item() == dec_pad:
    print(y_input)
    break


y_input_cpu = y_input.cpu().numpy()
words = [list(dec_vocab.keys())[list(dec_vocab.values()).index(idx)] for idx in y_input_cpu[0]]
# 拼接单词以形成句子
sentence = ' '.join(words)
print(sentence)
